function [rxDataBits,isConnected,toff,diagnostics] = helperOFDMRxEstReal(rxWaveform,sysParam,rxObj,pathGains,pathFilters,sampleTimes,txDiagnostics)
...
if ~camped
    ...
    % If ta >= 0, sync symbol was found. Adjust buffer timing so that
    ...
    [rxDataBits,diagnostics] = rxFrame(rxWaveform,sysParam,rxObj,pathGains,pathFilters,sampleTimes,txDiagnostics);
...
function [rxDataBits,diagnostics,count,error_count] = rxFrame(rxWaveform,sysParam,rxObj,pathGains,pathFilters,sampleTimes,txDiagnostics)
...
% frame
[estChannel,estChannel_original] = helperOFDMChannelEstimationEstReal(sysParam,pathGains,pathFilters,sampleTimes);

%% Opción 1 Demodular solo lo usado del símbolo

% dcIdx = (FFTLength/2)+1;
% nullLen = (FFTLength - 72)/2;
% headerNullInd = [1:nullLen dcIdx FFTLength-nullLen+2:FFTLength].';
% 
% % Extract out the header symbol samples and demod
% refSamples = cfoCorrectedData( ...
%     ((headerIdx-length(ssIdx))*numSampPerSym + (0:numSampPerSym-1)),:);
% demodulatedHeader = ...
%     ofdmdemod(refSamples,FFTLength,CPLength,symbOffset,headerNullInd);
% 
% demodulatedHeader = reshape(demodulatedHeader,[size(demodulatedHeader,1),size(demodulatedHeader,3)]);
% numLgSc = (FFTLength-usedSubCarr)/2;
% 
% pos = (nullLen+1-numLgSc:FFTLength-nullLen-numLgSc)+180*2;  
% 
% eqHeaderData = ofdmEqualize(demodulatedHeader(:,:,:),estChannel(pos,:,:),sysParam.nVar);
% 
% headerData = eqHeaderData;

%% Opción 2 Demodular todo el símbolo

% Extract the header symbol samples and demod
refSamples = cfoCorrectedData( ...
    ((headerIdx-length(ssIdx))*numSampPerSym + (0:numSampPerSym-1)));
...
%eqHeaderData = ofdmEqualize(demodulatedHeader,estChannel(pos,:,:),sysParam.nVar);
%eqHeaderData = ofdmEqualize(demodulatedHeader,estChannel_original(:,headerIdx-1),sysParam.nVar);
eqHeaderData = ofdmEqualize(demodulatedHeader,estChannel_original(:,headerIdx),sysParam.nVar,"DataFormat","2-D");
...
dataHeaderFrame = cfoCorrectedData(((4-length(ssIdx))*numSampPerSym + (0:(numSampPerSym*(sysParam.numSymPerFrame-3) - 1))));
...
equalizedData = ofdmEqualize(...
    demodulatedData(:,1:end,:),estChannel(sysParam.pos_data,:,:),sysParam.nVar);

% Equalize the pilots as well

equalizedPilots = ofdmEqualize(pilots(:,1:end,:),estChannel(sysParam.pos_pilots,:,:),sysParam.nVar);
...
for i = 1:size(headerData,3)
    [headerBits(:,i),headerCRCErrFlag] = ...
        OFDMHeaderRecovery(headerData(:,1,i),sysParam);
end
...
diagnostics = struct( ...
    'estCFO',freqOffset,...
    'estChannel',estChannel_original(:,3:end),...
...
if sysParam.print_allData == 1
    diagnostics.rxConstellationHeader_NoEq = demodulatedHeader(:);
    diagnostics.rxConstellationData_NoEq = demodulatedData(:,1:end);
    diagnostics.rxConstellationData_NoPhaseCorr = equalizedData;
end
...
% CRC check
[headerBits,errFlag] = crcDet(vitOut(1:(end-(sysParam.headerConvK-1))));
...
% Output CRC
[outBits,errFlag] = crcDet(dataScrOut);
