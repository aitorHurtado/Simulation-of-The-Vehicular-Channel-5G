...
toff = -1;  % by default, return a negative value in timing offset to indicate
...
if ~syncDetected
    ...
    [toff,packetDetected] = helperOFDMFrameSync(rxIn,sysParam);

    if packetDetected
        syncDetected = true;
        toff = toff - sysParam.FFTLen - sysParam.CPLen;
        fprintf('\nSync symbol found.\n');
