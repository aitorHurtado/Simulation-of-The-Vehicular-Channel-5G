function [sysParam,txParam] = helperOFDMSetParameters(userParam,antennaParam)
...
sysParam.print_allData = userParam.print_allData;
...
% MIMO data and pilots positions

%% Data and Pilots numbers
if strcmp(antennaParam.antennaSystem,"est_ofdmChannelResponse") == true
    dataIdx = setdiff(1:sysParam.usedSubCarr,1:sysParam.pilotSpacing:sysParam.usedSubCarr);
    sysParam.pos_data = zeros(1,length(dataIdx)* (sysParam.numSymPerFrame-3));
    
    index = 1;
    for i = 1:sysParam.numSymPerFrame-3
        for j = 1:length(dataIdx)
            sysParam.pos_data(index) = (3-1+i)*sysParam.usedSubCarr + dataIdx(j);
            index = index + 1; 
        end
    end
    ...
    sysParam.pos_pilots = zeros(1,length(pilotInd)* (sysParam.numSymPerFrame-3));

    index = 1;
    for i = 1:sysParam.numSymPerFrame-3
        for j = 1:length(pilotInd)
            sysParam.pos_pilots(index) = (3-1+i)*sysParam.usedSubCarr + pilotInd(j);
            index = index + 1; 
        end
    end
end
